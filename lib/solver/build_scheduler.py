...
from lib.solver import learning_schedules_fastai as lsf
import torch

def build_lr_scheduler(config, optimizer, total_step):
  ...
  config = config.optimizer.value

  if optimizer_type == 'rms_prop_optimizer':
    lr_scheduler = _create_learning_rate_scheduler(
      config.lr, optimizer, total_step=total_step)
...
def _create_learning_rate_scheduler(learning_rate_config, optimizer, total_step):
  ...
  config = learning_rate_config.value
  ...
  if learning_rate_type == 'multi_phase':
    ...
    lr_scheduler = lsf.OneCycle(
      optimizer, total_step, config.lr_max, config.moms, config.div_factor, config.pct_start)
