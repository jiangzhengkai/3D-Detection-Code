...
import torch
from lib.engine.convert_batch_to_device import convert_batch_to_device, convert_sequence_batch_to_device
...
def compute_on_dataset_sequence(model, data_loader, device):
    model.eval()
    results_dict = []
    cpu_device = torch.device("cpu")
    results_dict = {}
    for batch in tqdm(data_loader):
        example = convert_sequence_batch_to_device(batch, device=device)
        with torch.no_grad():
            outputs = model(example)
            for output in outputs:
                token = output['metadata']['token']
...
def test_sequence(dataloader, model, save_dir=None, device='cuda', distributed=False, logger=None):
    if distributed:
        model = model.module
    ...
    detections = compute_on_dataset_sequence(model, dataloader, device)
