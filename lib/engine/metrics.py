...
def get_metrics(config, cls_loss, loc_loss, cls_preds, labels, sampled, num_class):
    batch_size = cls_preds.shape[0]
    encode_background_as_zeros = config.model.loss.encode_background_as_zeros
    use_sigmoid_score = config.model.loss.use_sigmoid_score

    if not encode_background_as_zeros:
        num_class += 1
    ...
    rpn_cls_loss = rpn_cls_loss_function(cls_loss).numpy()[0]
    rpn_loc_loss = rpn_loc_loss_function(loc_loss).numpy()[0]
    ret = {
        "loss": {
    ...
    for i, thresh in enumerate(rpn_metrics_thresholds):
