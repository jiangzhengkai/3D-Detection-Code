...
import vtk
import numpy as np
from vtk_visualizer import visualizercontrol
from vtk_visualizer import pointobject
import sys
import os
import binascii
import struct
...
def render(args):
    ...
    oripoints = np.fromfile("./data_labels/10_points.bin", dtype=np.float32).reshape([-1, 5])[:, 1:]
    bboxes = np.fromfile("./data_labels/10_boxes_f64.bin", dtype=np.float64).reshape([-1, 7])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bin-path", type=str, required=True, help="path for lidar file")
