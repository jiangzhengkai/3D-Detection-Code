...
def display_single_lidar(data, gt_boxes=None, dt_boxes=None, points=0.2, view=False, idx=0):
...
def project_velo2camera(vel_data, Trv2c):
    homo_vel_data = np.hstack((vel_data[:, :3],
                               np.ones((vel_data.shape[0], 1),
                                       dtype='float32')))
    vel_data_c = np.dot(homo_vel_data, Trv2c.T)
    vel_data_c /= vel_data_c[:, -1].reshape((-1, 1))
    vel_data_c = np.hstack((vel_data_c[:, :3], vel_data[:, -1].reshape((-1,
                                                                        1))))
    return vel_data_c
...
def show_single_pc_vel(raw_lidar, gts, dts, Trv2c, rect, idx):
    ...
    display_single_lidar(raw_lidar, gts_lidar, dts_lidar, points=.01, view=True, idx=idx)
